...
# Page-specific rules only; palette, tables, .pos/.neg/.muted come from
# qscreen_ui.css() (prepended below).
_CSS = """
body{font:14px/1.5 var(--font);max-width:960px;margin:24px auto;padding:0 18px}
h1{font-size:21px} table{font-size:13px}
th,td{padding:6px 8px} td:nth-child(2){text-align:left}
.a{color:var(--neg);font-weight:700} .w{color:var(--warn-strong)}
.muted{font-size:12px} tr:hover{background:var(--accent-bg)}
"""
...
def render_html(board: dict) -> str:
    ...
    return ("<!doctype html><html lang='en'><head><meta charset='utf-8'><title>QSE watchlist</title>"
            "<style>" + qscreen_ui.css() + _CSS + "</style></head><body>" + "".join(P) + "</body></html>")
