...
# Page-specific rules only; the shared palette, tables, .pos/.neg/.muted/.tag/
# .fx/.rep and the flag list come from qscreen_ui.css() (prepended below).
_CSS = """
body{font:14px/1.55 var(--font);max-width:820px;margin:24px auto;padding:0 18px}
h1{font-size:22px;margin:0 0 2px} h2{font-size:16px;border-bottom:2px solid var(--border-soft);padding-bottom:4px;margin:26px 0 8px}
h3{font-size:13px;color:var(--fg-soft);margin:14px 0 4px;text-transform:capitalize}
.sub{color:var(--fg-soft);margin:0 0 4px} .muted{font-size:12px}
table{font-size:13px;margin:6px 0}
ul.tl{list-style:none;padding-left:0} ul.tl li{border-left:3px solid var(--accent-border);padding:2px 0 2px 8px;margin:3px 0}
td.base{background:var(--base-cell);font-weight:700}
.hl{font-size:20px;font-weight:700;margin:4px 0}
.charts{display:flex;gap:16px;flex-wrap:wrap;margin:10px 0}
svg.bars{border:1px solid var(--border-soft);border-radius:6px;background:#fff} svg.spark{vertical-align:middle}
"""
...
def _render_html(ctx, series, ratios, trends, flags, segments, valuation):
    ...
    return ("<!doctype html><html lang='en'><head><meta charset='utf-8'><title>"
            + E(ctx["name"]) + " — analyst report</title><style>" + qscreen_ui.css() + _CSS
